...
async function cascadeConvert(buffer, filename, toFormat) {
  ...
  // 1. Try each CloudConvert key in order
...
export default async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: err.message || 'Error interno del servidor en la conversión' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
