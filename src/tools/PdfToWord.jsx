...
export default function PdfToWord({ onBack }) {
  ...
  const handleFileSelected = ([selectedFile]) => {
    ...
    setCustomFilename(selectedFile.name.replace(/\.[^/.]+$/, '') + '_word');
  };
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const filename = (customFilename.trim() || 'documento_convertido') + '.docx';
      downloadFile(convertedBlob, filename, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');

      triggerConfetti();
      setProgress(null);
    } catch (err) {
      console.error('Error al convertir PDF a Word:', err);
  ...
  const handleDownloadAgain = () => {
    ...
    const filename = (customFilename.trim() || 'documento_convertido') + '.docx';
    downloadFile(downloadBlob, filename, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <ToolHeader
        ...
        description="Convierte tus archivos PDF a documentos Word (.docx) 100% editables con tablas, encabezados e imágenes perfectas."
        icon={FileText}
      ...
      {!file ? (
        <FileUploader
          ...
          title="Selecciona el archivo PDF para convertir a Word (.docx)"
          subtitle="Conversión de alta definición: párrafos, tablas, títulos y diseño original intactos"
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-rose-600 text-white flex items-center justify-center font-bold text-xs shadow-md shadow-rose-500/25">
                PDF
              </div>
              <div>
            ...
            <span className="text-xs font-semibold px-3 py-1 rounded-full bg-rose-50 dark:bg-rose-950/50 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900">
          ...
          <div className="p-6 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center gap-4">
            <DownloadFilenameInput
              ...
              extension=".docx"
              label="Nombre del documento Word al descargar:"
            />
            ...
            {downloadBlob ? (
              <div className="flex flex-col items-center gap-3 animate-in fade-in duration-300">
                <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 font-semibold text-sm">
                  ...
                  <span>¡Documento Word (.docx) generado y descargado exitosamente!</span>
                </div>
                ...
                >
                  ...
                  <span>Volver a Descargar Word (.docx)</span>
                </button>
              </div>
            ) : (
              <button
